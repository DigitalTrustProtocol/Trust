...
export class IndexScore implements IScore {
...
export class IndexScoreMap extends Map<number, IndexScore> {

  getSubject(subjectIndex: number, degree: number): IndexScore {
    let subjectScore = this.get(subjectIndex);
    if (!subjectScore) {
      subjectScore = new IndexScore();
